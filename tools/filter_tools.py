...
@tool(
    description=(
        "Filtra o dataset com uma condição e retorna estatísticas do subconjunto. "
        "Exemplos de condição: \"Position == 1\", \"season == 2023\", "
        "\"Team == 'Red Bull Racing'\", \"Points > 10\"."
    ),
    parameters={
        "type": "object",
        "properties": {
            "condicao": {
                "type": "string",
                "description": "Condição de filtro no formato pandas query. Ex: \"season == 2023\".",
            },
        },
        "required": ["condicao"],
    },
...
def filtrar(condicao: str) -> dict:
    ...
    if df_filtrado.empty:
        return {"erro": f"Nenhum registro encontrado para a condição: '{condicao}'"}
...
@tool(
    description=(
        "Filtra o dataset e retorna apenas estatísticas agregadas. "
        "NÃO retorna registros individuais, linhas, exemplos ou valores brutos."
        "Ideal para perguntas como: 'quem venceu mais corridas em 2023?' "
        "Use condicao para filtrar (ex: 'season == 2023 and Position == 1') "
        "e coluna_contagem para agrupar (ex: 'Driver')."
    ),
    parameters={
        "type": "object",
        "properties": {
            "condicao": {
                "type": "string",
                "description": "Condição de filtro no formato pandas query.",
            },
            "coluna_contagem": {
                "type": "string",
                "description": "Coluna categórica para contar ocorrências. Ex: 'Driver', 'Team'.",
            },
            "top_n": {
                "type": "integer",
                "description": "Quantos resultados retornar. Padrão: 10.",
            },
        },
        "required": ["condicao", "coluna_contagem"],
    },
)
def filtrar_e_contar(condicao: str, coluna_contagem: str, top_n: int = 10) -> dict:
    df = state.require_loaded()
    ...
    if df_filtrado.empty:
        return {"erro": f"Nenhum registro encontrado para: '{condicao}'"}

    if coluna_contagem not in df.columns:
        return {"erro": f"Coluna '{coluna_contagem}' não existe."}

    contagem = df_filtrado[coluna_contagem].value_counts().head(top_n)
    return {
        "condicao": condicao,
        "coluna_contagem": coluna_contagem,
        "total_registros": len(df_filtrado),
        "resultado": {str(k): int(v) for k, v in contagem.items()},
    }


@tool(
    description=(
        "Agrupa o dataset por uma coluna e aplica uma função de agregação em outra coluna. "
        "Funções disponíveis: mean, sum, min, max, count, median."
    ),
    parameters={
        "type": "object",
        "properties": {
            "grupo": {
                "type": "string",
                "description": "Nome da coluna para agrupar. Ex: 'Team', 'Driver', 'season'.",
            },
            "coluna": {
                "type": "string",
                "description": "Nome da coluna numérica para agregar. Ex: 'Points', 'Position'.",
            },
            "funcao": {
                "type": "string",
                "description": "Função de agregação: mean, sum, min, max, count, median.",
            },
            "top_n": {
                "type": "integer",
                "description": "Quantos resultados retornar ordenados de forma decrescente. Padrão: 10.",
            },
        },
...
def agrupar_e_agregar(grupo: str, coluna: str, funcao: str, top_n: int = 10) -> dict:
    ...
    funcoes_validas = ["mean", "sum", "min", "max", "count", "median"]
    if funcao not in funcoes_validas:
        return {"erro": f"Função '{funcao}' inválida. Use: {funcoes_validas}"}
    ...
    return {
        ...
        "coluna": coluna,
        "funcao": funcao,
        "resultado": {str(k): round(float(v), 3) for k, v in resultado.items()},
    }
...
@tool(
    description=(
        ...
        "Exemplos de uso: tempo total de corrida de um piloto, media de volta rapida por equipe."
    ),
    parameters={
        "type": "object",
        "properties": {
            ...
            "operacao": {
                "type": "string",
                "description": "Operacao a realizar: sum, mean, min, max, count.",
            },
            "condicao": {
                "type": "string",
...
def analisar_tempo(coluna: str, operacao: str, condicao: str = "") -> dict:
    ...
    if coluna not in colunas_validas:
        return {"erro": f"Coluna '{coluna}' nao suportada. Use: {colunas_validas}"}

    operacoes_validas = ["sum", "mean", "min", "max", "count"]
    if operacao not in operacoes_validas:
        return {"erro": f"Operacao '{operacao}' invalida. Use: {operacoes_validas}"}

    if condicao:
        try:
            df = df.query(condicao)
        except Exception as e:
            return {"erro": f"Condicao invalida: {e}"}
        if df.empty:
            return {"erro": f"Nenhum registro para a condicao: '{condicao}'"}
    ...
    if serie_seg.empty:
        return {"erro": f"Nenhum valor de tempo valido encontrado em '{coluna}' com os filtros aplicados."}
...
@tool(
    description=(
        "Retorna o tempo total de corrida de um piloto em uma corrida especifica. "
        ...
        "Se terminou X voltas atras do lider (+X lap/laps), informa quantas voltas de atraso. "
...
def tempo_total_piloto(piloto: str, circuito: str, temporada: int) -> dict:
    ...
    if corrida.empty:
        return {"erro": f"Corrida nao encontrada: {circuito} {temporada}. Verifique o nome do circuito e a temporada."}

    linha_piloto = corrida[corrida["Driver"] == piloto]
    if linha_piloto.empty:
        pilotos_disponiveis = corrida["Driver"].tolist()
        return {"erro": f"Piloto '{piloto}' nao encontrado em {circuito} {temporada}.", "pilotos_na_corrida": pilotos_disponiveis}
    ...
    # Caso: piloto nao terminou (DNF/DNS/DQ)
    ...
    if status in (3, 4, 5):
        return {
            ...
            "tempo_total": None,
            "observacao": f"O piloto nao concluiu a corrida (Status: {status_map.get(status)}).",
        }

    # Caso: "+X lap(s)" — terminou voltas atras
    ...
    if lap_match:
        ...
        return {
            ...
            "tempo_total": None,
            "observacao": f"O piloto terminou {n_laps} {'volta' if n_laps == 1 else 'voltas'} atras do lider. Tempo total exato nao disponivel.",
        }
    ...
    return {
        ...
        "observacao": "Nao foi possivel calcular o tempo total com os dados disponiveis.",
    }
